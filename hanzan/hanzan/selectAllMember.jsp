...
<center>
<table class="memb" cellpadding="10">
        <tr>
            <td>ID</td>
            <td>비밀번호</td>
            <td>성명</td>
...
<%
		while(rs.next()){
			String id2 = rs.getString("memId");   //  다음 각 함수들의 인수들은 테이블의 각 필드명들임
...
%>
   <tr>                     <!--  상세정보를 나타낼 페이지 링크부분에 회원 아이디를 함께 넘겨줌 -->
 	  <td> <a href="selectDetailMember.jsp?id=<%=id2%>"><%=id2%></a></td>
      <td><%=passwd%></td>
	  <td><%=name%></td>
	  <td><%=birth%>(<%=solar%>)</td>
	  <td><%=address1%><%=address2%></td>
 	  <td><%=phone%></td> 
  	  <td align=center><a href="deleteMember.jsp?id=<%=id2%>">Yes</a></td>
  </tr>
<%
		}  // while문의 끝
%>
